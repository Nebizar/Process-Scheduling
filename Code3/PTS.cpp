...
public:
	...
	void schedule()
	{
		...
		while (i < maxJobs)//for each element
		{
			if (data[i].submit > timeT)
			{
				timeT = data[i].submit;
			}
			while (data[i].submit <= timeT && i<maxJobs)//if ask is ready - to vector
			{
				ready.push_back(data[i]);
				i++;
				if (i >= maxJobs) break;
			...
			while (counterH<orgS)
			{
				...
				for (int j = 0; j < maxNodes; j++)
				{
					if (procTab[j] <= timeT) check++;
				}
				pom = 0;
				for (int j = 0; j < readyS; j++)
				{
					if (ready[j].proc <= check && min > check - ready[j].proc)
					{
						min = check - ready[j].proc;
						BPL[pom] = j;
						pom++;
				...
				if (pom > 0)
				{
					...
					chosen = BPL[pomCh];
					choice = true;
					//if(ready[chosen].proc<=check) choice = true;
				}

				if (choice)//if enough processors
				{
					//cout<<"good"<<endl;
					int finish = timeT + ready[chosen].run;
					counter = 0;
					for (int j = 0; j<maxNodes; j++)//insert finish time for every occupied processor
					{
						if (procTab[j] <= timeT)
				...
				else
				{
					...
					for (int j = 0; j < maxNodes; j++)
					{
						if (procTab[j]<min2 && procTab[j]>timeT)
							min2 = procTab[j];
					}
					timeT = min2;
					//cout << "Time: " << time << endl;
				...
				if (i < maxJobs)
				{
					while (data[i].submit <= timeT)//if ask is ready - to vector
					{
						ready.push_back(data[i]);
						i++;
						orgS++;
...
int main(int argc, char **argv)
{
	...
	ofstream plik;
	tasklist.scheduleFirst();
	best_output = tasklist.getV();
	for (int a = 0; a < best_output.size(); a++)
		best += best_output[a].start + best_output[a].stop;
	cout << best << endl;
	int count = 0;
	t1 = obliczSekundy(clock());
	while (count < 5 && obliczSekundy(clock())<290)
	{
		tasklist.schedule();
		out = tasklist.getV();
		for (int a = 0; a < out.size(); a++)
			sol += out[a].start + out[a].stop;
		cout << sol << endl;
		if (sol < best)
		{
			best = sol;
		...
		if (t2 - t1 > 300 - t2) break;
		t1 = t2;
	}
	cout << endl << best << endl;
	cout << obliczSekundy(clock()) << endl;
